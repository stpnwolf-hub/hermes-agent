...
class TestSubdirectoryHintTracker:
    ...
    def test_discovers_agents_md_in_subdirectory(self, project):
        """Reading a file in backend/ should discover backend/AGENTS.md."""
        tracker = SubdirectoryHintTracker(working_dir=str(project))
        result = tracker.check_tool_call(
            "read_file", {"path": str(project / "backend" / "src" / "main.py")}
        )
        # backend/src has no hints, but backend/ does — check if we discover it
        # Note: we check the directory of the file, which is backend/src/
        # backend/src/ has no AGENTS.md, so result should be None for that specific dir
        # But backend/ itself should be discovered when we access backend/AGENTS.md
        result2 = tracker.check_tool_call(
            "read_file", {"path": str(project / "backend" / "AGENTS.md")}
        )
        assert result2 is not None
        assert "Backend-specific instructions" in result2
